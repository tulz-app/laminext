package io.laminext.domtestutils.matching
...
class TestableHtmlAttr[V](val attr: HtmlAttr[V]) extends AnyVal {
  ...
  private[domtestutils] def nodeAttrIs(maybeExpectedValue: Option[V], sorted: Boolean = false)(
    node: dom.Node
  ): MaybeError = {
    node match {
        ...
        (maybeActualValue, maybeExpectedValue) match {
          ...
          case (Some(actualValue), Some(expectedValue)) =>
            ...
            if (actualValueMaybeSorted == expectedValueMaybeSorted) {
              ...
              Some(
                s"Attr `${attr.name}` value is incorrect: actual value ${repr(actualValueMaybeSorted)}, expected value ${repr(
                  expectedValueMaybeSorted
                )}.\nActual:\n${diffActual}\nExpected:\n${diffExpected}"
              )
            }
          ...
          case (Some(actualValue), None) =>
            Some(
              s"Attr `${attr.name}` should not be present: actual value ${repr(actualValue)}, expected to be missing"
            )
        }
      case _ =>
        Some(
          s"Unable to verify Attr `${attr.name}` because node $node is not a DOM HTML Element (might be a text node?)"
        )
    }
  }
