...
final class InputValidationOps(el: Input) {
  ...
  @inline def validatedFile[Err](
    ...
    ValidatedElement(
      el,
      el.files(changeStreamTransform).map(_.toSeq),
  ...
  @inline def validatedFiles[Err, Out](
    ...
    changeStreamTransform: EventStream[Event] => EventStream[Event] = identity
  ): ValidatedElement[html.Input, Seq[File], Err, Out] = ValidatedElement(el, el.files(changeStreamTransform).map(_.toSeq), validation)
